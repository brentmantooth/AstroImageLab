...
| `source_masked_background(data, box_size=64, fwhm_px=4.0, step=1, n_passes=SOURCEMASK_N_PASSES, user_exclusion=None, mesh=None, rms_mesh=None, progress_callback=None)` | `analysis/source_mask.py` | Section 3g's single entry point → `MaskedBackgroundResult`. Pass the caller's already-computed `mesh`/`rms_mesh` (every `AstroImage` has them after `estimate_background()`) to skip a redundant full `Background2D` pass. `user_exclusion` is a full-res bool mask applied **twice** — to the stage-1 cell selection and to the final mask — because those do different jobs (better pedestal vs. guaranteed exclusion). `.ok` is False when the mask left too little sky; `.fallback_reason` says why. Carries `rms_map` (the masked pass's full-res RMS, so an adopting caller does not pair a masked background with an unmasked noise floor) and `agreement` (`plane_quadric_agreement`'s record). Measured ~18 s on a 24 MP frame. `progress_callback`, if given, is a `Callable[[str], None]` fired at each of the four stage boundaries — threaded straight through by `AstroImage.estimate_background()` and consumed by `gui/bg_region_dialog.py`'s `_PreviewThread`/`gui/main_window.py`'s `_BgEstimateThread` to drive a GUI progress indicator instead of freezing silently for tens of seconds |
...
| Deriving a decimation stride from `mask.shape // view.shape` instead of reusing `decimation_step()` | `gui/bg_region_dialog.py`'s detector-mask overlay computed its own stride as `max(1, max(mask.shape[:2]) // max(h, w))`, while `_show_images()` had already decimated the *displayed* image with `decimation_step()`'s formula. The two strides drift apart under floor-division rounding whenever the image dimensions aren't evenly divisible by either result, so the mask overlay landed visibly offset from the region it was actually computed against — exactly the failure mode `decimation_step`'s own Key Utilities entry warns about ("two arrays that must land on the identical pixel grid... need to agree on `step` exactly"). Fixed by calling `decimation_step(*mask.shape[:2])` for the overlay too, so both arrays are decimated by construction-identical strides. Whenever a second array must overlay or be compared pixel-for-pixel against one already decimated via `decimation_step`, call `decimation_step` again for the second array rather than re-deriving a stride from a shape ratio — even a formula that looks equivalent can diverge by rounding. |
| Stretching a full-resolution array before decimating it for display | `gui/bg_region_dialog.py::_show_images()` originally called `img.display_image(stretch=True)` (an STF stretch, whose median/MAD scan runs over the *full* array) and only decimated the already-stretched result afterward — most of what made the dialog slow to open on a 24 MP frame, since the final displayed image is decimated anyway and the stretch statistics don't need full resolution to be representative. Fixed by decimating the raw data first (`img.data[::step, ::step]` via `decimation_step`) and stretching only that much smaller array (`normalize_for_display`). Whenever a display path both decimates and applies a full-array statistic (median/MAD/percentile), do the decimation first — the statistic's accuracy loss from computing it on the strided data is negligible, and the CPU cost isn't. |
