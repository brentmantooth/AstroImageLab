...
SECTION8_LOCALMAX_FOOTPRINT_MULT = 2.0           # multiplier on each metric's own characteristic scale (kernel px / sigma px / 2**level) used as the maximum_filter footprint (peak/non-max-suppression neighbourhood) for Section 8l local-maxima detection
SECTION8_LOCALMAX_PROMINENCE_PERCENTILE = 99.0   # percentile of the (smoothed) per-scale |A|,|B|-combined peak-source array used as the minimum local-maximum height (Section 8l)
SECTION8_LOCALMAX_PRESMOOTH_FRACTION = 0.5       # fraction of each metric's own scale used as the Gaussian pre-smoothing sigma applied to the peak-source array before maximum_filter detection, to suppress single-pixel noise-driven false maxima (Section 8l); detection only — masked-region statistics are still measured on the raw, unsmoothed maps
SECTION8_LOCALMAX_REGION_FRACTION = 0.5          # fraction of the maximum_filter footprint used as the binary_dilation radius (px) grown around each detected peak pixel, so the mask covers the local neighbourhood around a peak rather than a single pixel (Section 8l)
SECTION8_LOCALMAX_DIST_MAX_SAMPLES = 1000000     # per masked population, per scale — caps render cost of the Section 8l A/B distribution violin+box figure; mean/std/significance-test stats are computed from the full population, not this capped copy
SECTION8_LOCALMAX_TOP_PERCENT = 5.0              # percentile of Image A's or Image B's own pixel-value distribution unioned (OR) into the local-maxima mask, so broad bright plateaus are captured even when they never register as an isolated local-maximum peak (Section 8l). E.g. 5.0 -> top 5% of values in A OR top 5% in B are included regardless of the peak-detection result. Also used, on its own (no peak-detection union), as the correlation-plot/NC-score population for Section 8d-8j and 8k -- one knob now governs both 8l's combined peak-or-top-N% mask and 8d-8j/8k's plain top-N% mask
