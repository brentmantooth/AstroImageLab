...
from PyQt6.QtWidgets import (
    ...
    QCheckBox, QPushButton, QSplitter, QFrame, QSizePolicy,
)
...
from gui.inspector_widgets import (
    COMPARE_LABELS, DEFAULT_LINE, CrossSectionPlot, LinkedImageView,
    RatioHistogramPlot, cmap_for_panel, comparison_map, comparison_range,
    theme_colors, to_2d, value_range,
)
...
class DataInspector(QMainWindow):
    ...
    def __init__(self, npz_path: str | Path, dark_mode: bool = False, parent=None):
        ...
        self._syncing = False          # re-entrancy guard for the 3-way line sync
    ...
    def _build_ui(self) -> None:
        ...
        self._tool_combo.setMinimumWidth(170)
        ...
        corr_row.addWidget(self._placeholder("Correlation — in mask"))
        corr_row.addWidget(self._placeholder("Correlation — outside mask"))
    ...
    def _placeholder(self, title: str) -> QWidget:
        """Phase 2 slot, shown so the finished layout is visible while reviewing."""
        lbl = QLabel(f"{title}\n(Phase 2)")
        lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl.setStyleSheet("color: #777777; font-size: 9pt; border: 1px dashed #555555;")
    ...
    def _apply_tool(self) -> None:
        """Only the active tool's handles accept the mouse, and say so in the hint."""
        locked = self._tool != TOOL_LINE
        for view in self._views:
            view.set_line_locked(locked)
