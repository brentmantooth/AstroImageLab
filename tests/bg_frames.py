...
def make_bg_frame(*, shape: tuple[int, int] = DEFAULT_SHAPE,
    ...
    nebulae  : tuple of (cy, cx, radius_px, amplitude_adu) Gaussian blobs.
    n_stars  : Gaussian point sources at random positions.

    Returns (data, true_sky, truth) where `true_sky` is the noiseless sky
    surface (level + gradient, excluding nebulae and stars) and `truth` holds
    the component layers plus convenience masks.
    """
    h, w = shape
